...
difficulty by make a python server to serve encoded text and expect
correctly decoded result before returning the flag.
...
9.  Fake conditional branch. Instead of always branch by using `jmp`,
...
There are 3 stages:
-   Stage 1: `push rax` to go to next function
...
-   Stage 3: A ROP table that maps input string using `inc`, and a function that accumulates using `xor`, `add` and compares the result with stored data.

Following the correct control flow, we should see a series of character checks.
Reverse the check, we shall recover the whole flag.
...
(slowly stepping through each instruction is also possible).
...
if you would like to understand every parts instructions used this challenge.
...
then a lot of `st1`. After the loop, we can also see the `strlen` function is
...
library came with the compiler to recognize `<Escape>` key along with 
many [others](https://uefi.org/specs/UEFI/2.10/Apx_B_Console.html#efi-scan-codes-for-efi-simple-text-input-protocol)
