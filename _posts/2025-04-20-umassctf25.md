...
I hope all live players enjoy the experience or learned soemthing new. 
...
-   [void.zip](https://github.com/dungwinux/umassctf25-src/blob/main/void/static/void.zip)
...
-   windbg was able to run correctly, but failed to render disassembly and AVX registers (WinAPI issue?)
-   lldb and gdb also could run correctly and also failed to render AVX registers.
-   Only an unofficial build of gdb was able to detect AVX correctly (https://github.com/ssbssa/gdb)

Fascinatingly, an unexpected issue in player's solve was found out during the few final hours of the CTF. Due to my selection of flag, there is an overflow in BCD addition at the third-to-last character, making some brute force attempts failed.

### Solution

Diving through code, we can see our main program read input, run it through function at `0x004017a0`, and compare the output with a specific string. To move on, we need to decipher that function.
...
The switch of CPU mode could be recognized via the use of `retf` instructions. As most of the x86-32 instructions are the basis for x86-64, the majority of the disassembly are still readable. We should see the step 4 as:
...
The line with `invalid` starts with 0x27, which is the opcode for `DAA` [_Decimal Adjust AL After Addition_](https://www.felixcloutier.com/x86/daa). This opcode is used in conjunction with add to perform BCD addition. You can click the link to see more how it is implemented. AVX2 implementation can be seen roughly following the same idea, except the CF is not accounted since it is impossible to overflow with our encoding.

The first addend is our input converted into our encoding. The second addend can be reconstructed and represented through the following Python code (which I named, confusingly, offset):
...
With those understandings, we can reverse this by writing a BCD subtraction. This version of mine also includes underflow handling.
...
Running through the expected output, we can derive what the input (aka the flag) should be:
...
Files:
-   [pandora.zip](https://github.com/dungwinux/umassctf25-src/blob/main/pandora/static/pandora.zip)
...
While brainstorming ideas, I made are three observations of the zip file, which later become the three pillars of the challenge:
...
3. DEFLATE has compression ratio of [1032:1](https://zlib.net/zlib_tech.html) at most. Meanwhile, BZip2 works well with exceptionally high repetition of characters.

Originally, I planned to block unzipping by putting huge file right before the flag in the archive. However, 7-Zip sees compressing everything is inefficient and decides to _store_ instead of _compress_ the flag. Instead, I decided to create a sparse file, with the data stored at the beginning. But it would be boring to just put flag in one file, right? So I hide the flag among what compression algorithm can do best: plain text - which leads us to Project Gutenberg.

But the longest process is compressing eveything, which took me ~25 hours doing naively through 7-Zip. All build commands can be found in my GitHub.
...
[libzip](https://libzip.org/) comes with useful ziptool that you can use to seek to each entry and then stream the decompression. Since the rest are `\0` and would take long to decompress, you can exit program base on timeout.
