...
The goal is to have yourself collided with the `IS` block and have the cat push both of you to the door. Tracing the path shall give the flag.
...
To approach, try the initial known part of the flag.
...
If we use the file as dictionary and encode the given line of text, none of them is larger than 4000. This suggests the number in Base-xx is custom and not large.
...
Alternatively, you can put the file in docProps folder and update via CLI: `7z u blank_passwd.xlsx docProps/`.
We get the flag by reading the file after uploading to and downloading from the server.
...
From the booth on left corner, we find out about Birght Brussels Festival in Belgium, confirming the waffle existence. Searching about scooter also reveal Belgium as one possible location.
